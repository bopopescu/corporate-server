...
# 1. New subordinate
#    pull complete database from main
# 2. Main is degraded to subordinate
...
name = 'replication'
description = 'LDAP subordinate replication'
filter = '(objectClass=*)'  # default filter - may be overwritten later
attributes = []
modrdn = '1'

subordinate = listener.baseConfig['ldap/server/type'] == 'subordinate'

if listener.baseConfig['ldap/subordinate/filter']:
	filter = listener.baseConfig['ldap/subordinate/filter']
...
def update_schema(attr):
	...
	print >>fp, '# This schema was automatically replicated from the main server'
...
def initialize():
	...
	if not subordinate:
		ud.debug(ud.LISTENER, ud.INFO, 'replication: not subordinate')
		return 1
	clean()
