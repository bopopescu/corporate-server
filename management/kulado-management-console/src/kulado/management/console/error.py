...
class LDAP_ServerDown(UMC_Error):
	...
	def _error_msg(self):
		...
		if self._is_main:
			...
			yield ' * ' + _('Make sure the domain controller main is running and reachable from %s') % (self._fqdn,)
			yield ' * ' + _('Check if enough hard disk space and free RAM is available on this server and on the domain controller main or free some resources')
		yield ' * ' + _('Restart the LDAP service on the domain controller main either via "service slapd restart" on command line or with the UMC module "System services"')
