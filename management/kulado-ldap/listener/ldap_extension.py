...
def postrun():
	"""Restart LDAP server Main and mark new extension objects active"""
	global schema_handler, acl_handler

	server_role = listener.configRegistry.get('server/role')
	if not server_role == 'domaincontroller_main':
		if not acl_handler._todo_list:
			# In case of schema changes only restart slapd on Main
			return
		# Only set active flags on Main
